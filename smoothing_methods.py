...
y = data.data #tarihlere baktığımızda haftalık

y = y['co2'].resample('MS').mean() #yeniden örnekleyerek aylık olduğu frekans bilgisini verip ortalamasını alalım

y.isnull().sum() #kaç eksik değer var bakalım.
#bir önceki   değeri ya da bir  sonraki değeri  kullanarak eksikliği dolduralım
...
y_pred = ses_model.forecast(48)  #kaç adımlık  tahmin yapacaksın ? 48periyotluk

mean_absolute_error(test, y_pred) #test edilen setimizle tahmin edilen değerlerimizi kıyaslıyoruz
